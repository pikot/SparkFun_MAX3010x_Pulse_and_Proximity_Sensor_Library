...
dcFilter_t MAX30105::dcRemoval(float x, float prev_w, float alpha)
{
  dcFilter_t filtered;
...
float MAX30105::meanDiff(float M, meanDiffFilter_t* filterValues)
{
  float avg = 0;
...
void MAX30105::lowPassButterworthFilter( float x, butterworthFilter_t * filterResult )
{  
  filterResult->v[0] = filterResult->v[1];



  //Fs = 3200Hz and Fc = 10Hz
//    filterResult->v[1] = (9.722340545023300251e-3 * x)
//				 + (0.98055531890995339950 * filterResult->v[0]);

    //1600/ 10
	filterResult->v[1] = (1.925927420233569318e-2 * x)
				 + (0.96148145159532860671 * filterResult->v[0]);

//  1600 / 4
//	filterResult->v[1] = (7.792936291951547446e-3 * x)
//				 + (0.98441412741609690684 * filterResult->v[0]);
...
//Report the most recent IR value
float MAX30105::getFilteredIR(void)
{
    uint32_t rawIR = getIR();
...
*/
//    return lpbFilterIR.result;// meanDiffResIR;
    return meanDiffResIR;
}


bool MAX30105::detectPulse(float sensor_value, float treshhold = PULSE_MAX_THRESHOLD)
{
